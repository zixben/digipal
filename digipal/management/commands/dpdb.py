...
import os
import re
# from optparse import make_option  # Remove this import as it's no longer needed
from .utils import *
from digipal.models import Text, CatalogueNumber, Description, TextItemPart, Collation
from digipal.models import HistoricalItem, ItemPart, Image, Hand, CurrentItem, ItemPartType
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            'command',
            choices=['backup', 'restore', 'list', 'tables', 'fixseq', 'tidyup1', 'checkdata1', 'cleanlocus', 'drop_tables', 'pseudo_items', 'duplicate_cis', 'add_cartulary_his', 'merge_rf', 'merge_frg', 'version', 'build', 'setbuild', 'create_index'],
            help='Command to run'
        )
        parser.add_argument(
            '--db',
    ...
    def merge_frg_verso_into_recto(self, fv, fr):
        ...
        fr.locus = re.sub(r'\s*,?\s*recto\s*$', r'', fr.locus)
        fr.group_locus = ''
        fr.save()
    ...
    def merge_rf(self):
        ...
        * Find all the RF with a r and v
        ...
        for hi in HistoricalItem.objects.all():
            ...
            for rf in rfs:
                if rf.locus and re.search(r'(?i)verso', rf.locus):
                    # we got a verso, find the corresponding recto
                    for rf2 in rfs:
                        if (rf2.current_item == rf.current_item) and \
                                re.sub(r'\s', '', rf2.locus.lower()) == re.sub(r'\s', '', rf.locus.lower().replace('verso', 'recto')):
    ...
    def dropTables(self, options):
        ...
        for table in tables:
            if re.search(r'%s' % table_filter, table):
                print('DROP %s' % table)
                dropTable(con, table, self.is_dry_run())
                c += 1
    ...
    def showTables(self, options):
        ...
        for table in tables:
            ...
            if re.search(r'%s' % table_filter, table):
                c += 1
                count = sqlSelectCount(con, table)
                # find datetime field
...
#                     table_displays[table_key] = table_display
    ...
    def cleanlocus(self, item_partids, options):
        ...
        c = 0
        for image in images:
            image.locus = re.sub(r'(.*)(r|v)$', r'\1', image.locus)
    ...
    def tidyUp1(self, options):
                ...
                if re.search('(?i)(reference|evidence)', field_name):
                    ...
                    if recs_src:
                        ...
                        for rec in recs_src:
                            ...
                            sqlWrite(con_dst, ('update %s set %s = ' % (table_dst, field_name_dst)) + (
        ...
        model_class_names = '''
            ...
            Proportion
            '''
        model_class_names = re.findall(r'(\S+)', model_class_names)
    ...
    def process_pseudo_items(self):
        ...
        fix_sequences(db_alias='default', silent=True)
    ...
    def get_normalised_code(self, code):
        ...
        if (code.startswith('20-')) or (code.startswith('43-')):
            code = re.sub(r'\(\d+\)\s*$', r'', code)
        code = re.sub(r'(\s|,|\.|-|_|\u2013)+', r'_', code)
    ...
    def handle(self, *args, **options):
        ...
        if command == 'fixseq':
            ...
            c = fix_sequences(options.get('db', 'default'), True)
        ...
        if command == 'restore':
            ...
            if len(args) < 2:
                raise CommandError(
                    'Please provide the name of the backup you want to restore. Use "python manage.py dpdb list".')
